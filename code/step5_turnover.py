...
from AP‑Tree (cleaned, K=20 & K=40) and TripleSort64 (cleaned) strategies,
...
CONFIG = {
    ...
    'TRIPLE_MAP_DIR': str(PROJECT_ROOT / 'output' / 'stock_mappings' / 'Triple64_cleaned'),
    ...
    'TRIPLE_WEIGHT_FILE': str(PROJECT_ROOT / 'output' / 'pruned' / 'TripleSort64_cleaned' / 'All_TripleSort_Detailed_Results.csv'),
    ...
    'TREE_DEPTH': 4,
    'Q_NUM': 2,
    'SORT_DIMS': (4, 4, 4),   # for TripleSort64
...
def generate_triple_mappings(config):
    print("\n=== Phase 1: Generate TripleSort64 stock mappings (cleaned sample) ===")
    other_feats = config['OTHER_FEATURES']
    combo_size = 2
    all_combos = list(combinations(other_feats, combo_size))
...
def read_combo_weights_triple(weight_file_path, section_name):
    """Read portfolio weights for TripleSort64 (no weight scheme)."""
    df = pd.read_csv(weight_file_path)
    if 'Type' in df.columns:
        type_col = 'Type'
        sec_col = 'Section' if 'Section' in df.columns else '截面名称'
...
def generate_all_weights(config):
    ...
    for sec_id in config['SECTION_IDS']:
        ...
        # TripleSort64 cleaned (no weight schemes)
            ...
            if Path(wf).exists():
                ...
                if weights:
                    df_w = build_weights_table('Triple', section_name, config['TRIPLE_MAP_DIR'],
                                                weights, test_start, test_end)
                    if df_w is not None:
                        out_path = out_dir / f"Triple64_cleaned_{section_name}_weights.csv"
                        df_w.to_csv(out_path, float_format='%.10f')
                        print(f"  ✓ Triple64: {df_w.shape[0]} stocks × {df_w.shape[1]} months")
                        avg_turn = compute_turnover(df_w)
                        turnover_records.append({
                            'Model': 'Triple64_cleaned',
                            'Section': section_name,
                            'Weight_Scheme': 'none',
                            'Average_Monthly_Turnover': avg_turn
                        })
                    else:
                        print(f"  ⚠ Triple64: no weight table")
                else:
                    print(f"  ⚠ Triple64: no selected portfolios")
            else:
                print(f"  ✗ Triple64 weight file missing")
